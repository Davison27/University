...
int main(int argc, char **argv)
{
	...
	do
	{
		...
		// Recibir el mensaje
		bytes_read = mq_receive(mq_server, readBuffer, MAX_SIZE, NULL); //metemos en la variable bytes_read el mensaje recibido del servidor
		// Comprar que la recepción es correcta (bytes leidos no son negativos)
		if (bytes_read < 0) //Comprobación de errores
		{
		...
		if (mq_send(mq_client, writeBuffer, MAX_SIZE, 0) != 0) //enviar el mensaje del buffer por la cola del cliente
		{
	...
	if (mq_unlink(serverQueue) == (mqd_t)-1) //borrar la cola server
	{
	...
	// Eliminar la cola del servidor
	if (mq_unlink(clientQueue) == (mqd_t)-1) //borrar la cola client
	{
