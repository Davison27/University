...
void Ruleta::escribeJugadores(){
    ...
    f.open("jugadores.txt",std::ios::out);                                      //Abrimos el fichero denominado "jugadores.txt"
...
void Ruleta::leeJugadores(){
    jugadores_.empty();                                                         //Limpiamos la lista de jugadores
    ...
    while (!f.eof()){                                                           //Mientras no lleguemos al final del fichero vamos a seguir leyendo datos
        ...
        getline(f, cad,',');
        jugador.setCodigo(cad);                                                 //Guardamos el primer parámetro en la lista 
        getline(f, cad,',');
        jugador.setNombre(cad);                                                 //Guardamos el primer parámetro en la lista 
        getline(f, cad,',');
        jugador.setApellidos(cad);                                              //Guardamos el primer parámetro en la lista 
        getline(f, cad,',');
        jugador.setDireccion(cad);                                              //Guardamos el primer parámetro en la lista 
        getline(f, cad,',');
        jugador.setLocalidad(cad);                                              //Guardamos el primer parámetro en la lista 
        getline(f, cad,',');
        jugador.setProvincia(cad);                                              //Guardamos el primer parámetro en la lista 
        getline(f, cad,',');
        jugador.setPais(cad);                                                   //Guardamos el primer parámetro en la lista 
        getline(f, cad,'\n');
        jugador.setDinero(std::stoi(cad));                                      //Guardamos el primer parámetro en la lista 
        jugadores_.push_back(jugador);                                          //Ponemos 
        getline(f, cad,',');
    }
...
void Ruleta::getPremios(){
    ...
    for(e=jugadores_.begin(); e!=jugadores_.end(); e++){                        //Bucle que recorrerá la lista de jugadores desde el principio hasta el final
        ...
        for(auto f=aux.begin(); f!=aux.end(); f++){
            Apuesta apuesta=*f;                                                 //Guardamos la apuesta actual en apuesta
            switch(apuesta.tipo){
                case 1:
...
bool Ruleta::esRoja(int const &valor){                                          //A la función le pasamos los valores de la bola para saber si son rojas o no
    int NumRojos[18]={1,3,5,7,9,12,14,16,18,19,21,23,25,27,30,32,34,36};        //Declaramos un vector que guarda los valores en los que la bola es roja
